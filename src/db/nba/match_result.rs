...
impl NbaMatchResult {
    pub fn upsert(&self, conn: &Connection) -> rusqlite::Result<()> {
        conn.execute(
            "
            INSERT INTO nba_match_results (
                season_id, game_id, home_team_id, away_team_id, home_score, away_score, postseason
            )
    ...
    pub fn list_for_season(conn: &Connection, season_id: i64) -> rusqlite::Result<Vec<Self>> {
        let mut stmt = conn.prepare(
            "
            SELECT season_id, game_id, home_team_id, away_team_id, home_score, away_score, postseason
