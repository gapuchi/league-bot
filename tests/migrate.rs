...
#[test]
fn startup_migration_adds_nfl_postseason_column() {
    let conn = Connection::open_in_memory().unwrap();
    // A pre-postseason (v1) database: nfl_match_results exists without the column.
    conn.execute_batch(
        "
    ...
    let postseason_columns: i64 = conn
        .query_row(
        ...
        .unwrap();
    assert_eq!(postseason_columns, 1);
}
